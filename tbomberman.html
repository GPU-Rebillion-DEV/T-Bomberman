...
const stories = [
  { level: 0, text: "Welcome to T-Man's quest!\n\nControl T-Man with arrow keys or joystick (touchscreen), place bombs with spacebar or bomb button, destroy enemies, and collect power-ups to defeat the Tech Giants!\n\nPower-Ups:\n- Extra Bomb (+B): Increases max bombs.\n- Speed Boost (S): Increases movement speed.\n- Range Boost (R): Increases bomb explosion range.\n\nPress ENTER to continue." },
  ...
  { level: 61, text: "T-Man rallies a digital rebellion in the Server Depths! The Tech Giants unleash their ultimate weapon: bomb-dropping drones. T-Man fights fire with fire, determined to break their control.\n\nPress ENTER to continue." },
  { level: 81, text: "In the heart of the Data Core, T-Man faces the Tech Giants’ full might. Hordes of minions swarm, but T-Man’s heart shines bright. Can he defeat the monopolies and restore freedom?\n\nPress ENTER to continue." }
];
...
class PowerUp {
  ...
  draw() {
    ...
    text(symbol, 0, 0);
    pop();
    debug('PowerUp ' + this.type + ' drawn at ' + this.x + ',' + this.y);
  }
  ...
  update() {
    ...
    if (player && player.x === this.x && player.y === this.y) {
      if (this.type === 'extraBomb') {
        maxBombs++;
        debug('Collected Extra Bomb: maxBombs = ' + maxBombs);
      } else if (this.type === 'speedBoost') {
        playerSpeed += 0.5;
        debug('Collected Speed Boost: playerSpeed = ' + playerSpeed);
      } else {
        bombRange++;
        debug('Collected Range Boost: bombRange = ' + bombRange);
      }
      return false;
    }
    return true;
...
class Player {
  ...
  draw() {
    ...
    ellipse(0, -tileSize / 3, 12, 12);
    pop();
    debug('Player drawn at ' + this.x + ',' + this.y);
  }
  ...
  move() {
    ...
    this.moveTimer += playerSpeed * 0.1; // Kleinere Schritte für flüssige Bewegung
    if (this.moveTimer >= 1) {
      this.moveTimer -= 1;
      ...
      if (newX >= 0 && newX < gridSize && newY >= 0 && newY < gridSize && grid[newY][newX] === 0) {
        if (enemies.some(enemy => enemy.x === newX && enemy.y === newY)) {
          gameState = 'gameover';
          debug('Player killed by enemy collision at ' + newX + ',' + newY);
          return;
        }
        if (bombs.some(b => b.x === newX && b.y === newY)) {
          debug('Movement blocked by bomb at ' + newX + ',' + newY);
          return;
        }
        this.isMoving = true;
        this.x = newX;
    ...
    if (this.active) {
      ...
      if (player && abs(moveX) > 0.3 || abs(moveY) > 0.3) {
        if (abs(moveX) > abs(moveY)) {
          player.setDirection(moveX > 0 ? 1 : -1, 0);
        } else {
          player.setDirection(0, moveY > 0 ? 1 : -1);
        }
      }
    }
  }
...
class Enemy {
  ...
  draw() {
    ...
    pop();
    debug('Enemy ' + this.type + ' drawn at ' + this.x + ',' + this.y);
  }
    ...
    if (this.moveTimer >= 1) {
      ...
      if (this.intelligence < 25) {
        ...
        if (this.intelligence >= 50 && millis() - this.lastBombTime > bombCooldown * 2) {
          ...
          debug('Enemy ' + this.type + ' placed bomb');
        }
      }
      if (move) {
...
class Bomb {
  ...
  update() {
    ...
    if (this.timer <= 0 && !this.exploded) {
      ...
      debug('Bomb exploded at ' + this.x + ',' + this.y);
    }
  }
  ...
  draw() {
    ...
    ellipse(tileSize * 0.2, -tileSize * 0.3, 5, 5);
    stroke(0, 255, 128);
    line(tileSize * 0.1, -tileSize * 0.2, tileSize * 0.2, -tileSize * 0.3);
    noStroke();
    pop();
    debug('Bomb drawn at ' + this.x + ',' + this.y);
  }
}
...
class Explosion {
  ...
  draw() {
    ...
    text('Boom!', 0, 0);
    pop();
    debug('Explosion drawn at ' + this.x + ',' + this.y);
  }
  ...
  update() {
    ...
    for (let i = enemies.length - 1; i >= 0; i--) {
      ...
      if (enemy && (
        ...
        enemies.splice(i, 1);
        debug('Enemy killed at ' + enemy.x + ',' + enemy.y);
      }
    }
    ...
    if (player && (
      ...
      gameState = 'gameover';
      debug('Player killed by explosion');
    }
    ...
    for (let i = -this.range; i <= this.range; i++) {
      if (this.x + i >= 0 && this.x + i < gridSize && grid[this.y][this.x + i] === 2 && !isPathBlocked(this.x, this.y, this.x + i, this.y)) {
        ...
        if (random() < 0.3) {
          ...
          debug('PowerUp spawned at ' + (this.x + i) + ',' + this.y);
        }
        for (let j = 0; j < 5; j++) {
        ...
        debug('Wall destroyed at ' + (this.x + i) + ',' + this.y);
      }
      if (this.y + i >= 0 && this.y + i < gridSize && grid[this.y + i][this.x] === 2 && !isPathBlocked(this.x, this.y, this.x, this.y + i)) {
        ...
        if (random() < 0.3) {
          ...
          debug('PowerUp spawned at ' + this.x + ',' + (this.y + i));
        }
        for (let j = 0; j < 5; j++) {
        ...
        debug('Wall destroyed at ' + this.x + ',' + (this.y + i));
      }
    }
  }
}
...
function setup() {
  ...
  for (let i = 0; i < numEnemies; i++) {
    ...
    if (attempts <= 100) {
      enemies.push(new Enemy(x, y, enemyTypes[floor(random(enemyTypes.length))], level));
      debug('Enemy spawned at ' + x + ',' + y);
    }
  }
  powerUps = [];
  maxBombs = 3;
  ...
  bombRange = 2;
  fadeAlpha = 0;
  joystick = new Joystick(80, height - 80, 50);
  bombButton = new Button(width - 80, height - 80, 60);
...
function draw() {
  ...
  noStroke();
  if (frameCount % 60 === 0) {
    particles.push(new Particle(random(width), random(height), 0, 0, color(100, 150, 255, 50)));
  }
  ...
  if (storyState === 'tutorial' || storyState === 'intro' || storyState === 'level1complete') {
    ...
    textSize(18);
    textAlign(CENTER, CENTER);
    text(stories[storyIndex].text, 50, 50, width - 100, height - 100);
    return;
  }
  ...
  for (let i = 0; i < gridSize; i++) {
    for (let j = 0; j < gridSize; j++) {
      if (grid[i][j] === 1) {
        ...
        rect(j * tileSize + 5, i * tileSize + 5, tileSize - 10, tileSize - 10, 5);
      } else if (grid[i][j] === 2) {
        fill(colors.destructible);
        rect(j * tileSize, i * tileSize, tileSize, tileSize);
        fill(colors.destructibleHighlight);
        rect(j * tileSize + 10, i * tileSize + 10, tileSize - 20, tileSize - 20);
      }
    }
  }
  ...
  if (gameState === 'gameover') {
    ...
    let bx = width / 2 - 100;
    let by = height / 2 + 20;
    let hover = mouseX > bx && mouseX < bx + 200 && mouseY > by && mouseY < by + 70;
    fill(hover ? color(0, 255, 128, 220) : color(0, 255, 128));
    rect(bx, by, 200, 70, 10);
    fill(255, 255, 255, 100);
    textSize(20);
    text('Restart', width / 2 + 2, height / 2 + 37);
    fill(34, 57, 34);
    text('Restart', width / 2, height / 2 + 35);
    fill(255, 255, 255, 100);
    textSize(14);
    text('Press ENTER', width / 2 + 2, height / 2 + 57);
    fill(34, 57, 34);
    text('Press ENTER', width / 2, height / 2 + 55);
    noLoop();
  }
  ...
  if (gameState === 'victory') {
    ...
    textSize(18);
    textAlign(CENTER, CENTER);
    text('Victory! T-Man triumphs over the Tech Giants, freeing the digital world. Users celebrate, and T-Man dances under a starry sky. Play again?', 50, 50, width - 100, height - 100);
    let bx = width / 2 - 100;
    let by = height - 120;
    let hover = mouseX > bx && mouseX < bx + 200 && mouseY > by && mouseY < by + 70;
    fill(hover ? color(0, 255, 128, 220) : color(0, 255, 128));
    ...
    fill(255, 255, 255, 100);
    textSize(20);
    text('Restart', width / 2 + 2, height - 93);
    fill(34, 57, 34);
    text('Restart', width / 2, height - 95);
    fill(255, 255, 255, 100);
    textSize(14);
    text('Press ENTER', width / 2 + 2, height - 73);
    fill(34, 57, 34);
    text('Press ENTER', width / 2, height - 75);
    noLoop();
  }
  ...
  text('Move: Arrow Keys', width - 140, 35);
...
function keyReleased() {
  ...
  if (keyCode === UP_ARROW && player.moveDirection && player.moveDirection.dy === -1) player.moveDirection = null;
  else if (keyCode === DOWN_ARROW && player.moveDirection && player.moveDirection.dy === 1) player.moveDirection = null;
  else if (keyCode === LEFT_ARROW && player.moveDirection && player.moveDirection.dx === -1) player.moveDirection = null;
  else if (keyCode === RIGHT_ARROW && player.moveDirection && player.moveDirection.dx === 1) player.moveDirection = null;
}
  ...
  if (touches.length > 0) {
    ...
    if (joystick.isTouched(touchX, touchY)) {
      ...
      if (millis() - lastBombTime > bombCooldown && bombs.filter(b => !b.exploded).length < maxBombs) {
        bombs.push(new Bomb(player.x, player.y));
        lastBombTime = millis();
        debug('Bomb placed via touch at ' + player.x + ',' + player.y);
      }
    }
  }
  return false;
...
function touchEnded() {
  ...
  if (player) player.moveDirection = null;
  return false;
}
...
function mousePressed() {
  if (gameState === 'gameover') {
    let bx = width / 2 - 100;
    let by = height / 2 + 20;
    if (mouseX > bx && mouseX < bx + 200 && mouseY > by && mouseY < by + 70) {
      restartGame();
    }
  } else if (gameState === 'victory') {
    let bx = width / 2 - 100;
